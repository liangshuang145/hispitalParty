  ...
  <el-dialog :value="value" name=" WorkIndicatorListDialog" :visible.sync="isShow" :before-close="handleClose" :title="workData.name + ' 指标列表'" width="750px"
    ...
    <el-row>
      <el-col>
        ...
        <el-button type="danger" icon="el-icon-delete" @click="delWorkIndicator" size="small">删除工作指标</el-button>
      </el-col>
    </el-row>
    ...
    <work-indicator-particulars-dialog :type="workIndicatorParticularsType" v-model="workIndicatorParticularsShow" :fatherWorkData="fatherWorkData"></work-indicator-particulars-dialog>
  </el-dialog>
</template>
...
//                    return
//                  }
                this.fatherWorkData = this.workData;
              ...
              delWorkIndicator(){
                ...
                this.$confirm('此操作将永久删除该工作指标, 是否继续?', '提示', {
                  confirmButtonText: '确定',
                  cancelButtonText: '取消',
                  type: 'warning'
                }).then(() => {
                  WorkService.delWorkIndicator({id:this.tableCheckedData.id}).then((res) => {
                    if(res.code === 200){
