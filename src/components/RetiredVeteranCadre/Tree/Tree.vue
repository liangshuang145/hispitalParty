...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions([
        'getSubjectList',
        'selectUserInfoLtxlgbListByDepartId',
        'selectUserInfoLtxlgbListByGroupId'
      ]),
      ...
      nodeClick(data, node) {
        if(node.level >= 3){ // 判断节点等级
          if(node.data.type === '部门'){ // 部门
            this.selectUserInfoLtxlgbListByDepartId({id:node.data.id})
          }else if(node.data.type === '行政'){// 行政
            this.selectUserInfoLtxlgbListByGroupId({id:node.data.id})
          }
        }
